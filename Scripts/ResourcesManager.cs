...
public class ResourcesManager : MonoBehaviour
{
    ...
    private Dictionary<string, SceneObj> activeObjDic;//<suid,SceneObj>    //两个库   一个是需要加载的库
    private Dictionary<string, SceneObj> inActiveObjDic;//<suid,SceneObj>   //直接显示的库
    ...
    private bool MoveToActive(ObjData obj)
    {
        ...
        if (InActiveObjDic.TryGetValue(obj.sUid, out sceneObj))
        {
            ...
            ActiveObjDic.Add(obj.sUid, sceneObj);  //添加到显示库
            InActiveObjDic.Remove(obj.sUid);  //隐藏库中删除
            return true;
        }
        return false;
    }
    ...
    {
        if (CheckIsActive(obj.sUid) != null)   //判断场景中是否有这个物体
        {
            return;
        }
        if (!MoveToActive(obj))  //判断是否显示出来
        {
